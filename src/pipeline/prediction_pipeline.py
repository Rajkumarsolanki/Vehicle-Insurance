...
class VehicleData:
    ...
    def get_vehicle_input_data_frame(self) -> DataFrame:
        """
        Returns DataFrame from VehicleData inputs
        """
        try:
    ...
    def get_vehicle_data_as_dict(self):
        """
        Returns dictionary from VehicleData inputs
        """
        logging.info("Entered get_vehicle_data_as_dict method in VehicleData class")
        ...
        try:
            input_data = {
                ...
                # Keep UI friendly names; will be mapped later
            ...
            logging.info("Exited get_vehicle_data_as_dict method in VehicleData class")
            return input_data
...
class VehicleDataClassifier:
    ...
    def predict(self, dataframe: DataFrame) -> str:
        """
        Predict using VehicleDataClassifier
        """
        try:
            logging.info("Entered predict method of VehicleDataClassifier class")

            # ✅ Map user-friendly column names → training names
            if "Vehicle_Age_lt_1_Year" in dataframe.columns:
                dataframe["Vehicle_Age_< 1 Year"] = dataframe["Vehicle_Age_lt_1_Year"]
                dataframe.drop("Vehicle_Age_lt_1_Year", axis=1, inplace=True)

            if "Vehicle_Age_gt_2_Years" in dataframe.columns:
                dataframe["Vehicle_Age_> 2 Years"] = dataframe["Vehicle_Age_gt_2_Years"]
                dataframe.drop("Vehicle_Age_gt_2_Years", axis=1, inplace=True)

            # Always add missing 1-2 Year column if not provided
            if "Vehicle_Age_1-2 Year" not in dataframe.columns:
                dataframe["Vehicle_Age_1-2 Year"] = 0
